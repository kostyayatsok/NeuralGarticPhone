...
class Room:
    def __init__(self, id_in, chat_id):
        ...
        self.cycle_list = (
            []
        )  # Список ТГ-ИД игроков в порядке их обхода (тот самый "цикл")
        self.player_map = {}  # ТГ-ИД --> Player словарь
        self.cycle_map = (
            {}
        )  # ТГ-ИД --> ind словарь, где ind - индекс чела в массива cycle_list
        ...
        self.image_history = (
            {}
        )  # ТГ-ИД --> [картинка], хранит цепочки картинок для альбома
        self.text_history = (
            {}
        )  # ТГ-ИД --> [текст], хранит цепочки подписей к картинкам для альбома
    ...
    def build_timer(self, time, pending, label):
        if self.chat_id == 0:
            self.timer = PendingTimer(
                time, pending, label, self.player_map.copy().keys()
            )
        else:
    ...
    async def add_member(self, user_data, is_admin=False):
        ...
        await self.send_plain_all(
            "Игрок {0} зашёл в комнату!".format(user_data.username), send_to_group=True
        )
    ...
    async def reset_time(self, new_time):
        ...
        await self.send_plain_all(
            "Ведущий изменил лимит по времени! Новый лимит: {0} секунд".format(
                new_time
            ),
            send_to_group=True,
        )
    ...
    async def reset_quality(self, new_quality):
        ...
        await self.send_plain_all(
            "Ведущий изменил качество генерации! Новое значение: {0}".format(
                new_quality
            ),
            send_to_group=True,
        )
    ...
    async def remove_member(self, user_id, destroy_on_empty=True):
        ...
        await self.send_plain_all(
            "Игрок {0} покинул комнату!".format(username), send_to_group=True
        )
    ...
    async def start_game(self):
        ...
        for iteration in range(members * 2):
            ...
            if ind1 == ind2:
                continue
            self.cycle_list[ind1], self.cycle_list[ind2] = (
                self.cycle_list[ind2],
                self.cycle_list[ind1],
            )
    ...
    async def apply_text(self, user_data, text_data):
        ...
        if self.stage != 1:
            await BotAPI.send_plain_text(
                user_data.id, "Пока рано/поздно отправлять текст!"
            )
            return
        ...
        source_id = self.cycle_list[
            (self.cycle_map[user_id] - (self.round - 1) + members) % members
        ]
        self.text_history[source_id][
            -1
        ] = text_data  # изменяем последний текст в истории (текст текущего раунда)
    ...
    async def picture_round_start(self):
        ...
        for user_id in self.player_map.copy().keys():
            current_iter = (
                self.cycle_map[user_id] + self.round - 1 + members
            ) % members
            await self.albums[user_id].add_text_page(
                (len(self.text_history[user_id]) - 1) * 2,
                len(self.text_history) * 2 - 1,
                self.text_history[user_id][-1],
                self.cycle_list[current_iter],
            )
        ...
        for user_id in self.player_map.copy().keys():
            ...
            URL_first = (
                self.URL + "/add_text" + "?text=" + self.text_history[user_id][-1]
            )
            print(
                "generating picture for user {0} text is {1}".format(
                    self.cycle_list[current_iter], self.text_history[user_id][-1]
                )
            )
    ...
    async def picture_round_update(self):
        ...
        if len(self.images) == len(self.text_history):
            ...
            for i in range(cycle_len):
                ...
                to_id = self.cycle_list[
                    (i + self.round) % cycle_len
                ]  # текущий игрок цепочки
                img_name = self.image_history[from_id][-1]
                await self.albums[from_id].add_image_page(
                    (len(self.image_history[from_id])) * 2 - 1,
                    len(self.image_history) * 2 - 1,
                    self.image_history[from_id][-1],
                    0,
                )
                print("resend picture from {0} to {1}".format(from_id, to_id))
                await BotAPI.send_photo_with_text(
                    to_id, img_name, "Что на этой картинке?"
                )
    ...
    async def print_history(self):
        ...
        for i in range(len(self.cycle_list)):
            ...
            result = str(i + 1) + ": "
            for j in range(
                (len(self.image_history[user_id])) + len(self.text_history[user_id])
            ):
                # проходимся по её участникам
                if j % 2 == 0:
                    # это текст!
                    result += self.text_history[user_id][j // 2]
                    current_id = self.cycle_list[
                        (i + j // 2) % len(self.cycle_list)
                    ]  # текущий чел из цепочки
                    result += " ({0}) ---> ".format(
                        self.player_map[current_id].username
                    )
                else:
                    # а это картинка!
