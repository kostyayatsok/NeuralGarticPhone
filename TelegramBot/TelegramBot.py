...
async def join_command_internal(message: types.Message | types.CallbackQuery):
    if isinstance(message, types.Message):
        await CommandResolver.join_command(
            message.from_user, message.text.replace("/join ", "", 1)
        )
    elif isinstance(message, types.CallbackQuery):
        await CommandResolver.join_command(
            message.from_user, message.message.chat.id, in_chat=True
        )


@disp.message_handler(
    chat_type=[types.ChatType.GROUP, types.ChatType.CHANNEL], commands=["create"]
)
async def create_chat_callback(message: types.Message):
    room_id = await CommandResolver.create_command(
        message.from_user.id, message.chat.id, in_chat=True
    )
    if room_id == "":
        return
    await CommandResolver.join_command(
        message.from_user, message.chat.id, in_chat=True, is_admin=True
    )
    await botik.send_message(
        message.chat.id, "Присоединиться:", reply_markup=inline_kb1
    )
...
async def create_command_callback(message: types.Message):
    ...
    if room_id == "":
        return
    await CommandResolver.join_command(
        message.from_user, room_id, in_chat=False, is_admin=True
    )
...
async def time_command_callback(message: types.Message):
    await CommandResolver.time_command(
        message.from_user, message.text.replace("/time ", "", 1)
    )


@disp.message_handler(commands=["quality"])
async def quality_command_callback(message: types.Message):
    await CommandResolver.quality_command(
        message.from_user, message.text.replace("/quality ", "", 1)
    )
